...
- **GAP-009** — Render/Neon topology, backup, restore, rollback, and operational-ownership certificate. **Closed, 2026-09-21** (documentation half: `smp1-standalone-v1-render-neon-production-topology-2026-09-21.md`; operational half: see below). Diagnostics/backup-recovery HTTP non-publication is confirmed *intentional* (named "Pack 2.10C"/"Pack 2.11E" in `server.ts`'s own comments, matching frozen's own non-exposure of the equivalent capability) — no action needed, not a gap. `GET /health`'s hardcoded status was a real gap; resolved by adding a separate `GET /health/database` route backed by the already-existing `checkDatabaseHealth` probe, deliberately *not* modifying `/health` itself since that exact route is what Render's own deploy pipeline polls to decide a new deploy is live — verified by a regression test that `/health`'s response is unaffected. Full record: `smp1-gap-009-operational-observability-findings-2026-09-21.md` (Standalone repo), implementation commit `8ce1d9f`. Not registered as a GAP-001 pre-handover blocker, since it's operational/infrastructure posture rather than a Canon business-domain capability.
