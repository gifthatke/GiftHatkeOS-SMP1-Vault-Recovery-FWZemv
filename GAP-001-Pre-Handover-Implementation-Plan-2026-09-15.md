...
- **GAP-009** — Render/Neon topology, backup, restore, rollback, and operational-ownership certificate. Documentation half closed (`smp1-standalone-v1-render-neon-production-topology-2026-09-21.md`). **Operational half investigated, 2026-09-21, not closed**: the two observations this entry originally flagged were followed up directly. Diagnostics/backup-recovery HTTP non-publication is now confirmed *intentional* (named "Pack 2.10C"/"Pack 2.11E" in `server.ts`'s own comments, matching frozen's own non-exposure of the equivalent capability) — resolved, no action needed, not a gap. `GET /health`'s hardcoded status is confirmed a real gap (a working `checkDatabaseHealth` probe exists, just isn't wired in) but was deliberately not fixed directly, because that exact route is what Render's own deploy pipeline polls to decide a new deploy is live — wiring in a live DB check carries deploy-reliability risk none of this session's other fixes had, and three options (leave as-is / fix `/health` directly and accept the risk / add a separate lower-risk endpoint) are recorded without choosing one. Full record: `smp1-gap-009-operational-observability-findings-2026-09-21.md` (Standalone repo). Not registered as a GAP-001 pre-handover blocker, since it's operational/infrastructure posture rather than a Canon business-domain capability.
