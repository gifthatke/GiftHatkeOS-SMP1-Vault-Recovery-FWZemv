...
# Canon amendments applied, 2026-09-23 — the one remaining action from either decision brief, now also done

The lifecycle-vocabulary brief's four "amend Canon" decisions (Materials, Users, CRM, Products) were translated into exact proposed Canon text, reviewed, and applied the same day: `Canon-Amendment-Proposal-2026-09-23.md` (Obsidian vault, `01_GOVERNANCE/Canon-Reconciliation-2026-09-13/`). Each of `Domain-02/08/19/21-Stage-10-Original.md` now certifies the lifecycle actually built and operating, not the one that was never implemented — Materials and Products both reduced to Active → Archived; Users kept Suspended as its own state (the one flagged judgment call, specifically confirmed on review); CRM replaced wholesale with frozen's actual 9-stage payment-centric funnel plus a Lost exception, since that funnel is the real, certified, revenue-generating system, not an incomplete version of Canon's generic one. Full record, including the exact before/after text for each: the proposal document itself.

This was explicitly the one item every closing note in this report flagged as outside this report's or this session's own authority to perform — a Canon-governance action requiring separate, formal sign-off distinct from any SMP1/Standalone operational decision. That sign-off happened. Nothing further from the original GAP-001 closing statement's four categories of remaining work is outstanding.
